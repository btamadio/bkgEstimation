...
struct Config {
    ...
    vector<double> ptBins3;
    vector<double> ptBins4;
    vector<double> ptBins5;
};
...
void loadConfig(Config& config, char* configFile) {
    ...
    while (getline(fin, line)) {
        ...
        else if (line.find("ptBins3 ") != string::npos){
        	getline(iss,s,'{');
        	getline(iss,s,'}');
        	istringstream iss2(s);
        	while(iss2){
        		if(!getline(iss2,s,',')) break;
        		config.ptBins3.push_back(stod(s));
        	}
        }

        else if (line.find("ptBins4 ") != string::npos){
          getline(iss,s,'{');
          getline(iss,s,'}');
          istringstream iss2(s);
          while(iss2){
            if(!getline(iss2,s,',')) break;
            config.ptBins4.push_back(stod(s));
          }
        }

        else if (line.find("ptBins5 ") != string::npos){
          getline(iss,s,'{');
          getline(iss,s,'}');
          istringstream iss2(s);
          while(iss2){
            if(!getline(iss2,s,',')) break;
            config.ptBins5.push_back(stod(s));
          }
        }
    }
}
