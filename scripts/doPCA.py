...
for entry in range(t.GetEntries()):
    ...
    if entry % 10000 == 0:
        print 'entry %i' % entry
    if t.njet == njet and t.dEta > 1.4 and t.nbjet_Fix70 > 0:
        m+=1
        for i in range(len(t.jet_pt)):
            xList.append(t.jet_m.at(i)/t.jet_pt.at(i))
        for i in range(len(t.jet_pt)):
            xList.append(t.jet_pt.at(i))
        for i in range(len(t.jet_eta)):
            xList.append(abs(t.jet_eta.at(i)))
        for i in range(len(t.jet_NTrimSubjets)):
            xList.append(t.jet_NTrimSubjets.at(i))
        for i in range(len(t.jet_bmatched_Fix70)):
            xList.append(t.jet_bmatched_Fix70.at(i))
