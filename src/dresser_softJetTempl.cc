#include "dresser_softJetTempl.hh"
int main (int argc, char **argv){
 ...
 // ./dresser ${templates} ${PE_num} ${filename}
  ...
  TH2F* templGrid[3];
  templGrid[0] = (TH2F*)file->Get("templGrid_3j_b0");
  templGrid[1] = (TH2F*)file->Get("templGrid_4j_b0");
  templGrid[2] = (TH2F*)file->Get("templGrid_5j_b9");
  //assume same grid for b0 and b1

  const int nEtaBins = (const int)templGrid[0]->GetNbinsY();
  const int nPtBins3 = (const int)templGrid[0]->GetNbinsX();
  const int nPtBins4 = (const int)templGrid[1]->GetNbinsX();
  const int nPtBins5 = (const int)templGrid[2]->GetNbinsX();
  ...
  for(int k =0; k< 3; k++){
    for(int i = 0; i < nEtaBins; i++){
      for(int j = 0; j < nPtBins3; j++){
        ...
        h_mpt[0][k][i][j]= (TH1F*)file->Get(hname);
        if(isMC) fluctuateMCTemp(PE_loop,h_mpt[0][k][i][j]);
        if(!isMC) fluctuateDataTemp(PE_loop,h_mpt[0][k][i][j]);
      }
      for(int j = 0; j < nPtBins4; j++){
        hname = Form("templ_4j_b%i_etaBin%i_ptBin%i",b_tag[k],i+1,j+1);
        h_mpt[1][k][i][j]= (TH1F*)file->Get(hname);
        if(isMC) fluctuateMCTemp(PE_loop,h_mpt[1][k][i][j]);
        if(!isMC) fluctuateDataTemp(PE_loop,h_mpt[1][k][i][j]);
      }     
      for(int j = 0; j < nPtBins5; j++){
        hname = Form("templ_5j_b%i_etaBin%i_ptBin%i",b_tag[k],i+1,j+1);
        h_mpt[2][k][i][j]= (TH1F*)file->Get(hname);
        if(isMC) fluctuateMCTemp(PE_loop,h_mpt[2][k][i][j]);
        if(!isMC) fluctuateDataTemp(PE_loop,h_mpt[2][k][i][j]);
      }
    }
  }
  ...
  for( int nEvent = 0 ; nEvent < numev ; nEvent ++ ){  
   ...
   double dEta_cut = 1.4;
    ...
    for(int k = 0; k < min(4,njet); k++){
      ...
      //Nominal
      calc_mass(dressed_mass,h_mpt[0][tmp_ind][etaBin-1][ptBin-1],pt);
      MJ += dressed_mass;
      mass[k] = dressed_mass;
      ...
      calc_mass(dressed_mass,h_mpt[0][tmp_ind][etaBin-1][ptShiftBin-1],pt);
      ...
      else if(!config.doSimple && njet >= 5){
      if(config.tmpType5 != -9){ // grid specific
          if(config.tmpType5 == -1) tmp_ind = b_tag; //default
          if(config.tmpType5 > -1) tmp_ind = config.tmpType5; //b_tag_vec[ind]
          if(config.tmpType5 == 3) tmp_ind = (*p->jet_bmatched_Fix70)[k]; //b-matched templates
      }
      else{ //same for both grids
          if(config.tmpType == -1) tmp_ind = b_tag; //default
          if(config.tmpType > -1) tmp_ind = config.tmpType; //b_tag_vec[ind]
      ...
      dummyPt = min(pt,templGrid[2]->GetXaxis()->GetXmax()-0.001);
      dummyEta = min(fabs(eta),1.99);
      bin = templGrid[2]->Fill(dummyPt,dummyEta,0);
      templGrid[2]->GetBinXYZ(bin,ptBin,etaBin,zBin);
      
      //Nominal
      calc_mass(dressed_mass,h_mpt[2][tmp_ind][etaBin-1][ptBin-1],pt);
      MJ += dressed_mass;
      mass[k] = dressed_mass;

      //1Up
      ptShiftBin = ptBin+1;
      if(ptShiftBin > nPtBins5) ptShiftBin = nPtBins5;
      calc_mass(dressed_mass,h_mpt[2][tmp_ind][etaBin-1][ptShiftBin-1],pt);
      MJ_shiftUp += dressed_mass;
      mass_shiftUp[k] = dressed_mass;
      ...
      if(config.tmpType4 != -9){ // grid specific
          if(config.tmpType4 == -1) tmp_ind = b_tag; //default
          if(config.tmpType4 > -1) tmp_ind = config.tmpType4; //b_tag_vec[ind]
          if(config.tmpType4 == 3) tmp_ind = (*p->jet_bmatched_Fix70)[k]; //b-matched templates
      }
      else{ //same for both grids
          if(config.tmpType == -1) tmp_ind = b_tag; //default
          if(config.tmpType > -1) tmp_ind = config.tmpType; //b_tag_vec[ind]
      ...
      //Nominal
      calc_mass(dressed_mass,h_mpt[1][tmp_ind][etaBin-1][ptBin-1],pt);
      MJ += dressed_mass;
      mass[k] = dressed_mass;

      //1Up
      ptShiftBin = ptBin+1;
      if(ptShiftBin > nPtBins4) ptShiftBin = nPtBins4;
      calc_mass(dressed_mass,h_mpt[1][tmp_ind][etaBin-1][ptShiftBin-1],pt);
      MJ_shiftUp += dressed_mass;
      mass_shiftUp[k] = dressed_mass;
