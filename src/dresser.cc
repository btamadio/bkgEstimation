...
int main (int argc, char **argv){
  ...
  const int nEtaBins = (const int)templGrid->GetNbinsY();
  ...
  for( int nEvent = 0 ; nEvent < numev ; nEvent ++ ){  
   ...
   double dEta_cut = 1.4;
    ...
    for(int k = 0; k < min(4,njet); k++){
      ...
      bin = templGrid->Fill(dummyPt,dummyEta,0);
      templGrid->GetBinXYZ(bin,ptBin,etaBin,zBin);
      
      //Nominal
      calc_mass(dressed_mass,h_mpt[tmp_ind][etaBin-1][ptBin-1],pt);
      ...
      calc_mass(dressed_mass,h_mpt[tmp_ind][etaBin-1][ptShiftBin-1],pt);
      if(useSF && k == 3 && b_tag && njet >= 4){
