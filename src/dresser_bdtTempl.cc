...
int main (int argc, char **argv){
  ...
  const int nBDTBins = (const int)templGrid->GetNbinsY();
  ...
  for( int nEvent = 0 ; nEvent < numev ; nEvent ++ ){  
    ...
    int bdtBin,ptBin,bin,zBin,ptShiftBin;
    double dummyMJ, dummyMJ_up, dummyMJ_down, dummyMass, dummyMass_up, dummyMass_down;
    double dummyPt;

    //loop through jets, dress
    double MJ = 0, MJ_shiftUp = 0, MJ_shiftDown = 0, pt, BDT_score,dressed_mass;
    ...
    for(int k = 0; k < min(4,njet); k++){
      ...
      bin = templGrid->Fill(dummyPt,BDT_score,0);
      templGrid->GetBinXYZ(bin,ptBin,bdtBin,zBin);
      
      //Nominal
      calc_mass(dressed_mass,h_mpt[tmp_ind][bdtBin-1][ptBin-1],pt);
      ...
      calc_mass(dressed_mass,h_mpt[tmp_ind][bdtBin-1][ptShiftBin-1],pt);
      if(useSF && k == 3 && b_tag && njet >= 4){
